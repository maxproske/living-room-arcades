...
export const createMap = async (file: any, entitiesFile: any, player: any) => {
  const level = await fetch(file).then((response) => response.text());
  const entities = await fetch(entitiesFile).then((response) =>
    response.json()
  );
  ...
  const map = Array.from(Array(numLevelLines), () =>
...
export const createTextureIndex = async (file: any) => {
  const textureIndex: any = [];
  ...
  lines.forEach((line) => {
    ...
    let x = +symbolData[0];
    let y = +symbolData[1];
    const numVariations = +symbolData[2];
...
export const createPlayerTextureIndex = async (file: any) => {
  const textureIndex: any = [];
  ...
  lines.forEach((line) => {
    ...
    const x = +directionData[0];
    const y = +directionData[1];
    const numAnimations = +directionData[2];
